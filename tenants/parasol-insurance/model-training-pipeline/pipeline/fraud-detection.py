...
def get_data(download_url: str, data_output_path: OutputPath()):
...
def download_from_s3(download_from_s3_path: str, data_output_path: OutputPath()):
    import os
    import boto3
    import botocore
    ...
    file_in_s3 = download_from_s3_path

    try:
        print(f"Downloading {file_in_s3}")
        bucket.download_file(file_in_s3, data_output_path)
        print("Done! Downloaded to: ")
...
def pipeline(download_from_s3_path: str = ''):
    
    get_data_task = download_from_s3(download_from_s3_path = download_from_s3_path)
    kubernetes.use_secret_as_env(
    task=get_data_task,
    secret_name='aws-connection-ibm-s3',
    secret_key_to_env={
...
if __name__ == '__main__':
    compiler.Compiler().compile(
        ...
        pipeline_parameters={'download_from_s3_path': 'card_transdata.csv'}
    )